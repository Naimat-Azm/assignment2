                ...
                script {
                    if (params.DOCKER_TAG?.trim()) {
                        ...
                        withCredentials([usernamePassword(credentialsId: 'dockerhub-credentials', usernameVariable: 'USER', passwordVariable: 'PASS')]) {
                            try {
                                // Execute curl and check its exit code directly in the shell script
                                tagsJson = sh(
                                    script: '''
                                        tags_json=$(curl -s -u "$USER:$PASS" "https://hub.docker.com/v2/repositories/naimatazmdev/demoapp/tags?page_size=100")
                                        exit_code=$?
                                        if [ $exit_code -ne 0 ]; then
                                            echo "cURL command failed with exit code $exit_code" >&2
                                            exit $exit_code
                                        fi
                                        echo "$tags_json"
                                    ''',
                                ...
                                error("Failed to retrieve tags from DockerHub. cURL command failed: ${e.message}")
                            }
                        }
